...
async function startBot() {
    ...
    // معالجة قائمة الإعلانات
            if (interaction.isStringSelectMenu() && interaction.customId === 'advertisement_panel_select') {
                ...
                if (selectedValue === 'create_ad_ticket') {
                    ...
                    const infoEmbed = new EmbedBuilder()
                        ...
                        .setTitle('📢 تذكرة إعلان')
                        .setImage(ADS_TICKET_IMAGE)
                        .setDescription(`${opener} تم فتح تذكرة الإعلان بنجاح.\n\nالرجاء اختيار نوع الإعلان من القائمة بالأسفل.`);
                    
                    const closeBtn = new ButtonBuilder().setCustomId('ticket_close').setLabel('حذف التيكيت').setStyle(ButtonStyle.Danger);
                    const closeRow = new ActionRowBuilder().addComponents(closeBtn);
                    
                    await ticketChannel.send({ content: `<@&1432054713607782661>\n${opener}`, embeds: [infoEmbed], components: [closeRow] });
                    
                    // إرسال قائمة اختيار نوع الإعلان
                    const adTypeEmbed = new EmbedBuilder()
                        .setColor(0x000080)
                        .setTitle('اختر نوع الإعلان لاهنت')
                        .setDescription('الرجاء اختيار نوع الإعلان المناسب من القائمة');
                    ...
                    const adTypeSelect = new StringSelectMenuBuilder()
                        ...
                        .addOptions([
                            {
                                label: 'برود لأعضاء أونلاين',
                                description: 'السعر: 5m + البوت مايتبند',
                                value: 'broadcast_online',
                                emoji: '📡'
                            },
                            {
                                label: 'برود لكل أعضاء السيرفر',
                                description: 'يرسل للجميع - السعر: 35m',
                                value: 'broadcast_all',
                                emoji: '📢'
                            },
                            {
                                label: 'إعلان وسط فعالية 30sec',
                                description: 'السعر: 5m | لو أول مرة تدفع زيادة 3m',
                                value: 'event_30sec',
                                emoji: '⏱️'
                            },
                            {
                                label: 'إعلان وسط فعالية 90sec',
                                description: 'السعر: 8.5m | لو أول مرة تدفع زيادة 3m',
                                value: 'event_90sec',
                                emoji: '⏰'
                            },
                            {
                                label: 'نقل فعالية لسيرفرك',
                                description: 'على حسب تفاعلك - السعر: 14-35m',
                                value: 'transfer_event',
                                emoji: '🔄'
                            },
                            {
                                label: 'روم يوم واحد',
                                description: 'كاتقوري الإعلانات + منشن + قيف - السعر: 1m',
                                value: 'room_1day',
                                emoji: '📅'
                            },
                            {
                                label: 'روم 3 أيام',
                                description: 'كاتقوري الإعلانات + منشن + قيف - السعر: 2.5m',
                                value: 'room_3days',
                                emoji: '📆'
                            },
                            {
                                label: 'روم 5 أيام',
                                description: 'كاتقوري الإعلانات + منشن + قيف - السعر: 4m',
                                value: 'room_5days',
                                emoji: '🗓️'
                            },
                            {
                                label: 'روم 7 أيام',
                                description: 'كاتقوري الإعلانات + منشن + قيف - السعر: 5.5m',
                                value: 'room_7days',
                                emoji: '📋'
                            },
                            {
                                label: 'روم 14 يوم',
                                description: 'منشن + وسط + قريت بالروم + قيف - السعر: 16.5m',
                                value: 'room_14days',
                                emoji: '📊'
                            },
                            {
                                label: 'ضمان 100-200',
                                description: 'السعر: 25m',
                                value: 'guarantee_100_200',
                                emoji: '✅'
                            },
                            {
                                label: 'ضمان 200-1k',
                                description: 'السعر: 50m',
                                value: 'guarantee_200_1k',
                                emoji: '💯'
                            }
                        ]);

                    const adTypeRow = new ActionRowBuilder().addComponents(adTypeSelect);
                    
                    await ticketChannel.send({ embeds: [adTypeEmbed], components: [adTypeRow] });

                    // إرسال رسالة التنويهات
                    const noticeEmbed = new EmbedBuilder()
                        .setColor(0xFF6B00)
                        .setTitle('⚠️ تنويهات هامة')
                        .setDescription(
                            '**إعلانات الرومات في أول كاتجوري بالسيرفر**\n\n' +
                            '• إذا كنت ترغب في أن يكون الروم في غير الكاتجوري الأول، سيكون هناك زيادة **1m** مليون\n\n' +
                            '• إذا أردت <@1142808181626634261> أن يقوم بالمنشن للعضو في الروم، سيكون هناك زيادة **1m** مليون\n\n' +
                            '**زيادات المنشن:**\n' +
                            '1. زيادة منشن @here تدفع زيادة **600k** كريدت\n' +
                            '2. زيادة منشن @everyone تدفع زيادة **900k** كريدت\n\n' +
                            '**طرق الدفع المقبولة:**\n' +
            ...
            if (interaction.isStringSelectMenu() && interaction.customId === 'ad_type_select') {
                ...
                const adTypeNames = {
                    'broadcast_online': 'برود لأعضاء أونلاين - السعر: 5m + البوت مايتبند',
                    'broadcast_all': 'برود لكل أعضاء السيرفر - السعر: 35m',
                    'event_30sec': 'إعلان وسط فعالية 30sec - السعر: 5m (لو أول مرة تدفع زيادة 3m)',
                    'event_90sec': 'إعلان وسط فعالية 90sec - السعر: 8.5m (لو أول مرة تدفع زيادة 3m)',
                    'transfer_event': 'نقل فعالية لسيرفرك - السعر: 14-35m على حسب تفاعلك',
                    'room_1day': 'روم يوم واحد - كاتقوري الإعلانات + منشن + قيف - السعر: 1m',
                    'room_3days': 'روم 3 أيام - كاتقوري الإعلانات + منشن + قيف - السعر: 2.5m',
                    'room_5days': 'روم 5 أيام - كاتقوري الإعلانات + منشن + قيف - السعر: 4m',
                    'room_7days': 'روم 7 أيام - كاتقوري الإعلانات + منشن + قيف - السعر: 5.5m',
                    'room_14days': 'روم 14 يوم - منشن + وسط + قريت بالروم + قيف - السعر: 16.5m',
                    'guarantee_100_200': 'ضمان 100-200 - السعر: 25m',
                    'guarantee_200_1k': 'ضمان 200-1k - السعر: 50m'
                };

                const confirmEmbed = new EmbedBuilder()
                    .setColor(0x00FF00)
                    .setTitle('✅ تم اختيار نوع الإعلان')
                    .setDescription(`<@1142808181626634261>\n\n**نوع الإعلان:**\n${adTypeNames[selectedValue]}`)
                    .setFooter({ text: 'سيتم التواصل معك قريباً من قبل فريق الإدارة' })
                    .setTimestamp();
